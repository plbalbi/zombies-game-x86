...
\begin{document}
    ...
    Task-Switch, se produciría caso, fuego y destrucción), retornando de la interrupción. De lo contrario, hacemos un task-switch a la nueva tarea mediante un jump far con un selector al task-gate indicado (el `próximo índice' que se obtuvo\todo{¿Qué pasaba si hacias un task-switch a vos mismo? \textbf{CAOS FUEGO DESTRUCCION}}). El RPL de dicho selector debe ser 0, porque es el privilegio que necesitamos para iniciar la conmutación de tareas (el CPL también es 0, porque la interrupción del reloj está en un segmento de nivel 0).
